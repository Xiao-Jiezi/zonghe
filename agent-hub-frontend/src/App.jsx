...
import { 
  ...
  MessageSquare, Paperclip, Trash2, User
} from 'lucide-react';
...
// ========================
// 模块 1 - 4: 保持您原有的专业视图组件逻辑（代码折叠）
// ========================

const DeepResearchView = () => { /* 保持原样以缩短篇幅展示核心改动，实际渲染中这里会使用真实代码。为了完整性，下方照旧 */
  ...
  const handleStartResearch = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/research`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: topic })
      });
      const resData = await response.json();
      setReport(resData.data);
    } catch (error) { setReport(`请求失败: ${error.message}`); } finally { setLoading(false); }
  };
    ...
    <div className="h-full flex flex-col space-y-6 animate-in fade-in duration-700">
      <div className="bg-white rounded-2xl p-8 border border-zinc-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
        ...
        <div className="flex space-x-4">
          <input 
            type="text" value={topic} onChange={(e) => setTopic(e.target.value)} disabled={loading}
          ...
          <Button variant="primary" onClick={handleStartResearch} disabled={loading} className="px-8 whitespace-nowrap">
            {loading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Sparkles className="w-4 h-4 mr-2" />} 开始研究
          </Button>
        </div>
      </div>
      <div className="flex-1 bg-white border border-zinc-100 rounded-2xl flex flex-col shadow-[0_2px_10px_rgba(0,0,0,0.02)] overflow-hidden min-h-0">
        <div className="border-b border-zinc-100 px-8 py-5 flex items-center bg-zinc-50/50">
          <AlignLeft className="w-4 h-4 text-zinc-400 mr-2" /> <span className="text-[13px] font-semibold text-zinc-700 tracking-wide">研究报告总览</span>
        </div>
        <div className="p-10 overflow-y-auto flex-1 text-[15px] text-zinc-700 leading-loose whitespace-pre-wrap font-sans">
          {loading ? ( <div className="h-full flex flex-col items-center justify-center text-zinc-400"><Loader2 className="w-8 h-8 animate-spin mb-4 text-zinc-800" /><p className="text-sm">引擎正在深度检索...</p></div> ) : report ? report : ( <div className="h-full flex flex-col items-center justify-center text-zinc-300"><FileText className="w-10 h-10 mb-4 opacity-50" /><p className="text-sm">暂无数据</p></div> )}
        </div>
      </div>
    </div>
  );
...
const AutoCoderView = () => {
  ...
  const handleSend = async () => {
    ...
    setInput(''); setMessages(prev => [...prev, { role: 'user', content: userMsg }]); setLoading(true);

    try {
      const response = await fetch(`${API_BASE_URL}/api/coder`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: userMsg })
      });
      const resData = await response.json();
      const aiResponse = resData.data;

      let textPart = aiResponse; let codePart = '';
      ...
      if (aiResponse.includes('```')) {
        const parts = aiResponse.split('```'); textPart = parts[0];
        const rawCode = parts[1]; codePart = rawCode.substring(rawCode.indexOf('\n') + 1).trim();
        setCodeContent(codePart);
      }
      setMessages(prev => [...prev, { role: 'assistant', content: textPart || '代码已生成。' }]);
    } catch (error) { setMessages(prev => [...prev, { role: 'assistant', content: `[错误] ${error.message}` }]); } finally { setLoading(false); }
  };
    ...
    <div className="h-full flex gap-6 animate-in fade-in duration-700">
      <div className="w-[40%] min-w-[350px] border border-zinc-200/80 rounded-2xl flex flex-col bg-white shadow-sm overflow-hidden">
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-6 text-[14px] bg-white">
          {messages.map((msg, idx) => (
            <div key={idx} className="flex gap-4 items-start">
              ...
              <div className="bg-zinc-50 border border-zinc-100 rounded-2xl rounded-tl-sm p-4 text-zinc-700 flex-1 whitespace-pre-wrap">{msg.content}</div>
            </div>
          ))}
          {loading && ( <div className="flex gap-4 items-start animate-pulse"><div className="w-8 h-8 rounded-full bg-zinc-900 text-white flex items-center justify-center flex-shrink-0"><Loader2 className="w-4 h-4 animate-spin" /></div><div className="text-zinc-400 text-sm py-1.5">思考代码结构...</div></div> )}
        ...
        <div className="p-5 bg-white border-t border-zinc-100">
          <div className="relative border border-zinc-200 rounded-xl bg-zinc-50 focus-within:ring-1 focus-within:ring-zinc-900 focus-within:border-zinc-900 transition-all">
            <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } }} placeholder="描述业务需求..." className="w-full bg-transparent pl-4 pr-12 py-4 text-sm focus:outline-none resize-none h-24 text-zinc-800"></textarea>
            <button onClick={handleSend} disabled={loading} className="absolute right-3 bottom-3 p-2 bg-zinc-900 text-white rounded-lg hover:bg-zinc-800 disabled:opacity-50"><Send className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
      <div className="flex-1 border border-zinc-800 rounded-2xl flex flex-col bg-[#18181B] shadow-xl overflow-hidden relative">
        <div className="px-6 py-4 border-b border-zinc-800/80 flex justify-between items-center">
          <div className="text-[13px] text-zinc-400 font-mono flex items-center gap-2"><FileCode2 className="w-4 h-4 text-zinc-500" /> workspace.{fileExt}</div>
          <button className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-zinc-400 hover:text-white bg-zinc-800 rounded-lg" onClick={() => navigator.clipboard.writeText(codeContent)}><Copy className="w-3.5 h-3.5" /> 复制</button>
        </div>
        <div className="flex-1 overflow-y-auto font-mono text-[14px] text-[#D4D4D8] flex">
          ...
          <pre className="flex-1 py-6 px-6 whitespace-pre-wrap">{codeContent}</pre>
        </div>
      </div>
    </div>
  );
...
const VisionOCRView = () => {
  const [file, setFile] = useState(null); const [preview, setPreview] = useState(null); const [isProcessing, setIsProcessing] = useState(false); const [result, setResult] = useState(''); const fileInputRef = useRef(null);
  const handleFileChange = (e) => {
    const selected = e.target.files[0];
    if (selected) { setFile(selected); if (selected.type.startsWith('image/')) setPreview(URL.createObjectURL(selected)); else setPreview('document'); setResult(''); }
  };
  const startOCR = async () => {
    ...
    try {
      ...
      const data = await response.json(); setResult(data.data);
    } catch (error) { setResult(`处理失败：${error.message}`); } finally { setIsProcessing(false); }
  };

  return (
    <div className="h-full flex flex-col space-y-6 animate-in fade-in duration-700">
      <div className="border border-dashed border-zinc-300 rounded-2xl p-10 flex flex-col items-center justify-center bg-white hover:bg-zinc-50 cursor-pointer group" onClick={() => fileInputRef.current.click()}>
        <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/*,.pdf,.txt,.doc,.docx" />
        {!file ? ( <> <div className="bg-zinc-100 p-4 rounded-2xl mb-4 group-hover:scale-105 transition-transform"><UploadCloud className="w-8 h-8 text-zinc-700" /></div><p className="font-semibold text-zinc-900 text-[15px]">点击或拖拽上传文档图像</p></> ) : ( <div className="flex flex-col items-center"><CheckCircle2 className="w-10 h-10 text-zinc-900 mb-3" /><p className="font-semibold">{file.name}</p></div> )}
      </div>
      <div className="grid grid-cols-2 gap-6 flex-1 min-h-0">
        <div className="border border-zinc-100 rounded-2xl bg-white flex flex-col overflow-hidden">
          <div className="border-b border-zinc-100 px-8 py-5 flex justify-between items-center bg-zinc-50/50"><span className="text-[13px] font-semibold text-zinc-700">源文件预览区</span>{file && <Button variant="primary" onClick={startOCR} disabled={isProcessing} className="py-2 text-[12px]">{isProcessing ? '读取与分析中...' : '开始重组分析'}</Button>}</div>
          <div className="flex-1 flex items-center justify-center p-8 bg-zinc-50/30">{preview === 'document' ? <div className="text-zinc-400"><FileText className="w-12 h-12 mb-4"/>文档格式</div> : preview ? <img src={preview} alt="preview" className="max-h-[300px] object-contain rounded-xl border border-zinc-200" /> : <span className="text-zinc-400 text-sm">等待文件加载</span>}</div>
        </div>
        <div className="border border-zinc-100 rounded-2xl bg-white flex flex-col overflow-hidden">
          <div className="border-b border-zinc-100 px-8 py-5 text-[13px] font-semibold text-zinc-700 bg-zinc-50/50">自然语言重组结果</div>
          <div className="flex-1 p-10 overflow-y-auto text-[15px] text-zinc-700 leading-loose whitespace-pre-wrap">{isProcessing ? <div className="flex flex-col items-center text-zinc-400"><Loader2 className="w-8 h-8 animate-spin mb-4 text-zinc-800" />深度阅读中...</div> : result ? result : <div className="flex flex-col items-center text-zinc-300"><AlignLeft className="w-10 h-10 mb-4 opacity-50" />暂无结果</div>}</div>
        </div>
      </div>
    </div>
  );
...
const SpeechAnalyticsView = () => {
  const [isRecording, setIsRecording] = useState(false); const [isProcessing, setIsProcessing] = useState(false); const [result, setResult] = useState('');
  const mediaRecorderRef = useRef(null); const audioChunksRef = useRef([]); const fileInputRef = useRef(null);
  const handleAudioUpload = async (e) => { if (e.target.files[0]) await sendAudioToBackend(e.target.files[0]); };
  const toggleRecording = async () => {
    if (!isRecording) {
      try {
        ...
        mediaRecorderRef.current.onstop = async () => { const audioBlob = new Blob(audioChunksRef.current, { type: 'audio/webm' }); const file = new File([audioBlob], "recorded_audio.webm", { type: 'audio/webm' }); await sendAudioToBackend(file); };
        mediaRecorderRef.current.start(); setIsRecording(true); setResult('');
      } catch (err) { alert("无法访问麦克风"); }
    } else { mediaRecorderRef.current.stop(); setIsRecording(false); }
  };
  const sendAudioToBackend = async (audioFile) => {
    setIsProcessing(true); const formData = new FormData(); formData.append('file', audioFile);
    try { const response = await fetch(`${API_BASE_URL}/api/speech`, { method: 'POST', body: formData }); const data = await response.json(); setResult(data.data); } catch (error) { setResult(`分析失败: ${error.message}`); } finally { setIsProcessing(false); }
  };
  return (
    <div className="h-full flex flex-col space-y-6 animate-in fade-in duration-700">
      <div className="border border-zinc-100 rounded-2xl bg-white p-10 flex items-center justify-between">
        <div><h2 className="text-xl font-bold text-zinc-900 mb-2">智能语音语义剖析</h2><p className="text-[14px] text-zinc-500 max-w-lg">支持实时录音或上传音频文件。</p></div>
        <div className="flex gap-4 items-center">
          <input type="file" accept="audio/*" ref={fileInputRef} onChange={handleAudioUpload} className="hidden" />
          <Button variant="default" onClick={() => fileInputRef.current.click()} disabled={isRecording || isProcessing} className="px-6 h-12 rounded-xl"><FileAudio className="w-4 h-4 mr-2" />上传音频</Button>
          <Button variant={isRecording ? "danger" : "primary"} onClick={toggleRecording} disabled={isProcessing} className={`px-6 h-12 rounded-xl ${isRecording ? 'animate-pulse' : ''}`}>{isRecording ? <StopCircle className="w-4 h-4 mr-2" /> : <Mic className="w-4 h-4 mr-2" />}{isRecording ? '结束录制并分析' : '开始麦克风录音'}</Button>
        </div>
      </div>
      <div className="border border-zinc-100 rounded-2xl bg-white flex-1 min-h-0 flex flex-col">
        <div className="border-b border-zinc-100 px-8 py-5 font-semibold text-zinc-700 text-[13px] bg-zinc-50/50">语义剖析看板</div>
        <div className="p-10 flex-1 overflow-y-auto text-[15px] text-zinc-700 leading-loose whitespace-pre-wrap">{isProcessing ? <div className="flex flex-col items-center text-zinc-400"><Loader2 className="w-8 h-8 animate-spin mb-4 text-zinc-800" />提取中...</div> : result ? result : <div className="flex flex-col items-center text-zinc-300"><Activity className="w-10 h-10 mb-4 opacity-50" />等待分析</div>}</div>
      </div>
    </div>
  );
};
...
// ========================
// 模块 5: 全新的综合聊天大模型视图 (像 Gemini 的交互)
// ========================
    ...
    if (!isRecording) {
      try {
        ...
        mediaRecorderRef.current.onstop = () => {
          ...
          const audioFile = new File([audioBlob], "录音文件_record.webm", { type: 'audio/webm' });
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (resData.status === 'success') {
        ...
        setMessages(prev => [...prev, { role: 'assistant', content: `[请求拒绝] ${resData.data}` }]);
      }
    } catch (error) {
      setMessages(prev => [...prev, { role: 'assistant', content: `[系统错误] ${error.message}` }]);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="h-full flex flex-col bg-white border border-zinc-200/80 rounded-2xl shadow-sm overflow-hidden animate-in fade-in duration-700 relative">
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-zinc-50/30">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-zinc-400 space-y-4">
            ...
            <p className="text-[15px] font-medium text-zinc-500">欢迎使用全能 AI 问答终端</p>
            <p className="text-[13px] max-w-sm text-center">在下方选择你需要的特定能力，发送文本、图片、PDF 或语音，获取定制化分析解答。</p>
          </div>
        )}
        ...
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-[75%] rounded-2xl p-5 text-[15px] leading-relaxed whitespace-pre-wrap shadow-sm
        ...
        {loading && (
          <div className="flex gap-4 items-start animate-pulse">
            <div className="w-9 h-9 rounded-full bg-zinc-900 text-white flex items-center justify-center"><Loader2 className="w-4 h-4 animate-spin" /></div>
            <div className="bg-white border border-zinc-100 rounded-2xl rounded-tl-sm px-6 py-4 text-zinc-400 text-sm">正在处理您的请求...</div>
          </div>
        )}
        <div ref={messagesEndRef} />
      </div>
      ...
      <div className="p-6 bg-white border-t border-zinc-100">
        <div className="max-w-4xl mx-auto relative border border-zinc-200 rounded-2xl bg-zinc-50 focus-within:ring-1 focus-within:ring-zinc-900 focus-within:border-zinc-900 transition-all shadow-sm">
          ...
          <div className="flex items-center justify-between px-4 pt-3 pb-1 border-b border-zinc-100/50">
            <div className="flex items-center gap-3">
              <select 
                value={mode} 
                onChange={(e) => { setMode(e.target.value); setFile(null); }}
              ...
              {file && (
                <div className="flex items-center gap-2 bg-emerald-50 text-emerald-700 border border-emerald-100 px-3 py-1 rounded-md text-[12px] font-medium animate-in zoom-in-95 duration-200">
                  ...
                  <button onClick={() => setFile(null)} className="ml-1 hover:text-emerald-900"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              )}
            </div>
          </div>
          ...
          <textarea 
            ...
            placeholder={
              mode === 'ocr' ? "请在左下方上传图片或文档，并在此输入您的要求..." :
              mode === 'speech' ? "请上传音频或使用麦克风录音，并输入附加分析要求..." :
          ...
          <div className="absolute left-4 bottom-4 flex gap-2 items-center">
            ...
            {['ocr', 'speech'].includes(mode) && (
              <>
                <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept={mode === 'ocr' ? "image/*,.pdf,.doc,.docx,.txt" : "audio/*"} />
                <button 
            ...
            {mode === 'speech' && (
              ...
              >
                ...
                <span className="text-xs font-medium">{isRecording ? '点击结束录音' : '录制语音'}</span>
              </button>
            )}
          </div>
        ...
        <div className="text-center mt-3 text-[12px] text-zinc-400">
          AI 生成的内容可能存在偏差，请按需选择最匹配的智能体模式以获得最佳效果。
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-zinc-100 p-4 lg:p-8 flex items-center justify-center font-sans text-zinc-900 selection:bg-zinc-200">
      <div className="w-full max-w-[1600px] h-[90vh] bg-white rounded-[2rem] shadow-2xl overflow-hidden flex border border-zinc-200/60 relative">
        ...
        <div className="w-[260px] bg-zinc-50/80 backdrop-blur-3xl border-r border-zinc-200/80 flex flex-col z-10 shrink-0">
          ...
          <div className="px-8 pt-8 pb-3 text-[11px] font-bold text-zinc-400 uppercase tracking-widest">系统核心模块</div>
